...
public class MemberController {
    ...
    @PostMapping
    public ResponseEntity<ApiResponse> handleSignUp(@RequestBody SaveMemberRequest requestDto) {
        MemberInfoResponse responseDataDto = memberService.save(requestDto);
        ApiResponse response = new ApiResponse(true, "회원가입 성공", responseDataDto);
        return ResponseEntity.status(HttpStatus.CREATED).body(response);
    }

    @GetMapping
    public ResponseEntity<ApiResponse> handleGetAllMembers() {
        List<MemberInfoResponse> responseDataDtoList = memberService.getMemberInfoAll();
        ApiResponse response = new ApiResponse(true, "모든 회원정보 조회 결과", responseDataDtoList);
//        return ResponseEntity.status(HttpStatus.OK).body(response);
    ...
    @GetMapping("/{id}")
    public ResponseEntity<ApiResponse> handleGetMemberById(@PathVariable Long id) {
        MemberInfoResponse responseDataDto = memberService.getMemberInfo(id);
        ApiResponse response = new ApiResponse(true, "회원정보 조회 결과", responseDataDto);
        return ResponseEntity.status(HttpStatus.OK).body(response);
    }

    // 실제로는 토큰이 인증된 사용자로 제한해야 합니다.
    @PutMapping("/{id}")
    public ResponseEntity<ApiResponse> handleUpdateMember(@PathVariable Long id, @RequestBody SaveMemberRequest requestDto) {
        MemberInfoResponse responseDataDto = memberService.update(id, requestDto);
        ApiResponse response = new ApiResponse(true, "회원 정보 수정 성공", responseDataDto);
        return ResponseEntity.status(HttpStatus.OK).body(response);
    }
    ...
    public ResponseEntity<Object> handleDeleteMember(@PathVariable Long id) {
        ...
        ApiResponse response = new ApiResponse(true, "회원 삭제 성공", responseDataDto);
        return ResponseEntity.status(HttpStatus.OK).body(response);
    }
